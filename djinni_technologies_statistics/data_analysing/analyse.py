...
def analyze_skills():
    ...
    skills_data.plot.bar(
        x='Skill', y='Total', rot=90, legend=False, alpha=0.5
    )
    plt.xlabel('Skills')
    plt.ylabel('Total')
...
def analyze_applicants():
    ...
    top_applicants = (
        applicants_data.nlargest(50, 'applications').
...
def perform_correlation_analysis(vacancies_file, skills_file, top_n=50):
    ...
    sns.heatmap(
        combined_matrix, annot=True,
        xticklabels=columns, yticklabels=top_skills, cmap='coolwarm'
    )
